...
export default function DeckReviewPage() {
  ...
  async function handleVerify() {
    if (!tokenStr || verifying) return;
    setVerifying(true);
    try {
      const res = await fetch("/api/deck-review/verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ token: tokenStr }),
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(typeof data.error === "string" ? data.error : "verify failed");
      toast.success(t("deckReview.verifiedToast"));
      await load();
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <AppHeader
        ...
        nav={
          <>
            <AppHeaderLink href="/community">{t("community.title")}</AppHeaderLink>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {canVerify ? (
              <button
                type="button"
                onClick={() => {
                  if (!window.confirm(t("deckReview.confirmVerify"))) return;
                  void handleVerify();
                }}
                disabled={verifying || awaitingAuthorRevision}
                className="btn-primary text-sm disabled:opacity-50"
                title={t("deckReview.verifyHint")}
              >
                {verifying ? t("deckReview.verifying") : t("deckReview.markVerified")}
              </button>
            ) : null}
          </div>
        </div>
