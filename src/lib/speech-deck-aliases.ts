...
export function migrateDeckSttAliases(fromDeckId: string, toDeckId: string): void {
  if (typeof window === "undefined") return;
  try {
    if (!fromDeckId || !toDeckId || fromDeckId === toDeckId) return;
    const fromKey = `${STORAGE_PREFIX}${fromDeckId}`;
    const toKey = `${STORAGE_PREFIX}${toDeckId}`;
    const fromRaw = localStorage.getItem(fromKey);
    if (!fromRaw) return;
    const toRaw = localStorage.getItem(toKey);
    if (toRaw) return; // don't overwrite existing
